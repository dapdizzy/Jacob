defmodule Jacob.Bot do
  ...
  def process_remote_script_execution(msg, channel, slack) do
    ~r/^\s*remote\s+execute\s+(?<command>[\s\w_\.\-\"\'\`!]+)\s+on\s+(?<destination>[\w\._]+)/miu
      |> Regex.named_captures(msg)
      |> case do
        nil -> nil
        %{"command" => command, "destination" => destination} ->
          exec_command = "execute " <> command
          correlation_id = RemoteExecutionSender |> RabbitMQSender.send_message(destination, exec_command, true)
  ...
  def process_how_is_he_doing(msg, channel, slack) do
    ...
    case rex |> Regex.named_captures(msg) do
      ...
      %{"service_name" => service_name} ->
        ...
        if service_state |> String.trim != "" do
          "Service *#{service_name_expanded}* is now *#{service_state}*"
        else
          "Service *#{service_name_expanded}* does not exist"
        end
    end
  end
